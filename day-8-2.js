/*--------------------------
  Made at: 17-12-2021
  Dificulty: 1/10
  Time to solve: ~5hours
  Solution explanation: 
     I had a lot of trouble with this one, majority because I forgot to define the digit 0 on the configuration.

     The idea is having a configuration, then I have 4 unique numbers (1,4,7,8) that I know how many segments it takes.
     With this information, I resolve each of these 4 numbers.
     After that, I go trough every NON UNIQUE numbers and try to check the segments that are shared between the unique numbers.
     And BAM! It works :)
---------------------------*/
...
inputs.forEach((input) => {
  //First step: Find all the segments to generate each digit for this input
  var dataIn = input[0].split(" ");

  //Filter all the uniques and find the segments for then
  digitsConfig.filter(x => x.isUnique).forEach((digitCfg) => {
    digitCfg.segments = dataIn.find((data) => data.length == digitCfg.nOfSegments).split("");
  })

  //For all the digits that are not unique, process based on the sharedSegments
  digitsConfig.filter(x => !x.isUnique).forEach((digitCfg) => {
    var possibleSegments = dataIn.filter((data) => data.length == digitCfg.nOfSegments);
    possibleSegments.some((possibleSegment) => {
      possibleSegmentsData = possibleSegment.split("")
      if(digitCfg.sharedSegments){
        var isIn = true;
       
        for (const [key, value] of Object.entries(digitCfg.sharedSegments)) {
          var unique = digitsConfig.find(x => x.digit === parseInt(key));
          var qntEqual = possibleSegmentsData.filter((s) => unique.segments.indexOf(s) > -1).length;
          if(qntEqual != value)
            isIn = false
        }

        if(isIn){
          digitCfg.segments = possibleSegmentsData;
          return true;
        }
      }
    })
  ...
  //Second step: Using the second part of the data, find the corellated digit based on the segments
  var dataOut = input[1].split(" ");
  var outputDigits = [];

  
  dataOut.forEach((data) => {
    var dataChars = data.split("").sort();
    digitsConfig.some((cfg) => {
      if(cfg.segments.length === dataChars.length){
        debugger;
        if(cfg.segments.sort().join("") == dataChars.join("")){
          outputDigits.push(cfg.digit)
          return true;
        }
      }
    })
  ...
  if(outputDigits.length != 4)
    throw new Error("something is wrong")

  var num = outputDigits.reduce((c, d) => c+d, '');
  console.info(num)
  generalTotal += parseInt(num, 10);
})

console.info(generalTotal);

//The correct answer is: 1028926